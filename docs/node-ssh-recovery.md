...
Download the new Windows/macOS/Linux script from the dashboard and run **enable** once
...
更新 VPS 只更新面板提供的下载文件，不会替换设备上的旧脚本。重新下载 Windows/macOS/Linux
...
- Linux uses a 30-second systemd timer, supports Ubuntu socket activation, and validates actual listeners before reporting success. Failed changes restore the previous configuration and managed SSH firewall rules without reloading the whole firewalld ruleset.
...
- Linux 使用 systemd，每 30 秒检查一次；兼容 Ubuntu socket 启动，实际监听校验通过才报告成功。失败时恢复旧配置及 SSH 专用防火墙规则，不整体重载 firewalld。
